...
- Support Organization selection, Team search and focus, lifecycle filtering, viewport navigation, and Team details.
...
Orientation, view choice, inactive visibility, search text, and expansion state are presentation state. Organization and focal-Team URL parameters must be normalized against loaded data and never be trusted as authorization or relationship input.
...
Search is local to the already authorized selected-Organization payload and matches Team display names case-insensitively. It yields a stable result list; choosing a result makes it the focal Team, reconstructs and fits its complete Organization-to-Team ancestor chain, clears optional expansions, and opens its details. Nonmatching Teams are not automatically revealed, while every authoritative ancestor remains visible.
...
The page component owns normalized control state and delegates node-card and detail presentation to focused Svelte components. Browser tests cover selection/focus, URL Organization switching, view fallback, controls, and mobile layout; unit/component tests cover projections and accessible content.
