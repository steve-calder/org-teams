...
- **THEN** the system selects one Team from that Person's ordinary memberships and managed Teams in stable Organization-name, Organization-UUID, Team-name, and Team-UUID order
...
The chart SHALL display at most one selected Organization and one focal Team at a time, SHALL offer all Organizations to every authenticated user in stable display-name and UUID order, and SHALL preserve valid Organization and focal-Team selection in a shareable URL that still requires authentication. An unknown or mismatched identifier SHALL NOT expose unrelated data and SHALL fall back to a valid deterministic selection or an explicit Organization-only state.
...
#### Scenario: Authenticated user opens a focal Team URL

- **WHEN** an authenticated user opens a chart URL containing matching valid Organization and Team identifiers
- **THEN** the matching Organization and Team are selected and the Team's complete ancestor path is presented

#### Scenario: Focal Team does not belong to selected Organization

- **WHEN** URL or local presentation state identifies a Team outside the selected Organization
- **THEN** the system discards that focal Team without returning or connecting unrelated Organization data
...
#### Scenario: Hidden intermediate ancestor exists

- **WHEN** a focal Team has one or more authoritative parent Teams
...
Each Team node SHALL present its name, friendly type, lifecycle status, manager summary, participant count, and subordinate disclosure state, where the participant count includes ordinary memberships and includes the implicit manager once when assigned. Selecting or finding a Team SHALL make it the focal Team and present an informational details panel containing its parent, direct children, and the same summary context. Team nodes and details SHALL NOT link to administration routes or expose hierarchy-editing controls.

#### Scenario: Authenticated user selects a Team node

- **WHEN** an authenticated user selects a Team in the chart
- **THEN** the Team is visually identified and described in an informational details panel without an administration link or editing control
- **AND** the chart reconstructs and fits the selected Team together with its complete ancestor path through the visual Organization root
- **AND** does not automatically reveal its descendants
...
The chart SHALL initially reveal only the Organization root and the focal Team's complete ancestor path when a focal Team exists. The Organization and Team nodes SHALL provide presentation-only disclosure controls that reveal direct subordinate Teams one level at a time without mutating hierarchy data. Unrelated roots, siblings, and descendants SHALL remain hidden until explicitly revealed.
...
- **WHEN** the authenticated user collapses the Organization while a focal Team exists
- **THEN** optional top-level Teams are hidden while the top-level Team required by the focal path remains visible
...
- **AND** choosing a result makes that Team focal and reveals its complete ancestor path without automatically revealing siblings or descendants
...
- **THEN** inactive Team nodes and their associated edges are omitted from both views and any now-hidden focal selection or expansion state is cleared

### Requirement: Navigable and accessible chart experience

The visual chart SHALL provide fit-to-view, zoom, pan, and keyboard-operable disclosure controls and remain usable across supported desktop pointer and keyboard input. The experience SHALL also provide a semantic nested-list tree view containing equivalent focal hierarchy, disclosure behavior, and summary context, and SHALL preserve that tree as a functional alternative when the interactive canvas is unavailable or unsuitable for the viewport.
...
- **THEN** the system presents the currently revealed hierarchy as keyboard-navigable nested semantic lists with selectable Team names, accessible disclosure controls, and equivalent type, status, manager, and participant context without administration links
