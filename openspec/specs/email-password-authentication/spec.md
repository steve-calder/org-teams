...
When the application runs in development mode, it SHALL idempotently provision a Better Auth account with email `dev@org-teams.local` and password `password`, ensure that account has one linked Person, and display the credentials on `/login`. Provisioning the account SHALL NOT establish a session for the anonymous request that triggers it. Outside development mode, the application SHALL NOT provision the default account or return its credentials in login page data.
...
- **THEN** the default Better Auth account exists with one linked Person and can authenticate with the specified credentials
...
- **THEN** provisioning succeeds without creating a duplicate authentication user or linked Person
...
- **THEN** it neither provisions the default account nor returns or displays the default credentials
